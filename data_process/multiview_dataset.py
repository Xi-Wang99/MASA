...
class train_dataset(Dataset):
    def __init__(self, args, mode):  # mode='train' or 'valid'
        ...
        for root, dirs, files in os.walk(data_root3):
            ...
            for file in files:
                ...
                imgs_top.append(img_top)

        imgs_front = torch.stack(imgs_front)
        imgs_side = torch.stack(imgs_side)
        imgs_top = torch.stack(imgs_top)
        ...
        # print('All labels are:{}'.format(label))
        ...
        for i, label_value in enumerate(label):
            if label_value <= 15:
                train_label.append(label_value.item())
                train_imgs_front.append(imgs_front[i])
                train_imgs_side.append(imgs_side[i])
                train_imgs_top.append(imgs_top[i])
            elif 16 <= label_value <= 20:
                valid_label.append(label_value.item())
                valid_imgs_front.append(imgs_front[i])
                valid_imgs_side.append(imgs_side[i])
                valid_imgs_top.append(imgs_top[i])

        # print(train_label)
        # print(valid_label)
        self.train_imgs_front = torch.stack(train_imgs_front)
        self.train_imgs_side = torch.stack(train_imgs_side)
        self.train_imgs_top = torch.stack(train_imgs_top)
        self.train_label = torch.tensor(train_label)
        self.valid_imgs_front = torch.stack(valid_imgs_front)
        self.valid_imgs_side = torch.stack(valid_imgs_side)
        self.valid_imgs_top = torch.stack(valid_imgs_top)
        self.valid_label = torch.tensor(valid_label)



    def __getitem__(self, i):
        if self.mode == 'train':
            imgs_front, imgs_side, imgs_top, label = self.train_imgs_front[i], self.train_imgs_side[i], self.train_imgs_top[i], self.train_label[i]
        elif self.mode == 'valid':
            imgs_front, imgs_side, imgs_top, label = self.valid_imgs_front[i], self.valid_imgs_side[i], self.valid_imgs_top[i], self.valid_label[i]
...
class test_dataset(Dataset):
    def __init__(self,args):
        ...
        for index in range(len(label_str)):
            ...
            # print('index', index, 'label', label[index], 'tmp', tmp)
        label = [int(x) for x in label]
        # print('test labels are:{}'.format(label))
        label = torch.tensor(label)
